...
public class TestUtils {
    ...
    TestUtils() throws IOException {
        fs = new FileInputStream(ConfigConstants.EXCELPATH);
    ...
    public static void getRunStatus() throws Exception {
        try {
            fs = new FileInputStream(ConfigConstants.EXCELPATH);
            workbook = new XSSFWorkbook(fs);
            sheet = workbook.getSheet(ConfigConstants.RUNMANAGERSHEET);
            for (int i = 1; i <= getLastRowNum(ConfigConstants.RUNMANAGERSHEET); i++) {
                testCases.add(getCellContent(ConfigConstants.RUNMANAGERSHEET, i, "TestCaseName"));
                testDescription.add(getCellContent(ConfigConstants.RUNMANAGERSHEET, i, "Test Case Description"));
                runStatus.add(getCellContent(ConfigConstants.RUNMANAGERSHEET, i, "Execute"));
                invocationCount.add(getCellContent(ConfigConstants.RUNMANAGERSHEET, i, "InvocationCount"));
                priority.add(getCellContent(ConfigConstants.RUNMANAGERSHEET, i, "Priority"));
            }
    ...
    public static Object[][] supplyDataForIterations(Method m) throws IOException {
        return getDataForDataprovider(ConfigConstants.EXCELPATH, ConfigConstants.TESTDATASHEETNAME, m.getName());
    }
